 ...
 * Creates demo users + 6 restaurants with menu items
 */
...
// ─── helpers ────────────────────────────────────────────────────────────────
const img = (keywords) =>
  `https://source.unsplash.com/800x600/?${encodeURIComponent(keywords)},food`;

const restaurantImg = (keywords) =>
  `https://source.unsplash.com/1200x600/?${encodeURIComponent(keywords)},restaurant`;
...
async function seed() {
  ...
  if (!uri || uri === 'local') {
    ...
    console.log('🔧 Starting embedded local MongoDB for seeding...');
    mongod = await MongoMemoryServer.create({ instance: { dbName: 'foodapp', port: 27017 } });
    uri = mongod.getUri();
  }
  ...
  const ownerUser = users.find(u => u.role === 'restaurant');
  console.log(`👥 Created ${users.length} demo users`);
  console.log('   📧 Admin:      admin@quickbite.com     / admin123');
  console.log('   📧 Owner:      owner@quickbite.com     / owner123');
  console.log('   📧 Delivery:   delivery@quickbite.com  / delivery123');
  console.log('   📧 Customer:   customer@quickbite.com  / customer123');

  // Create restaurants + menus
  for (const data of RESTAURANTS) {
    const { menu: menuData, ...restaurantData } = data;
    const restaurant = await Restaurant.create({ ...restaurantData, owner: ownerUser._id, status: 'approved' });
    const menuItems = await MenuItem.insertMany(menuData.map(item => ({ ...item, restaurant: restaurant._id })));
    console.log(`🍽️  ${restaurant.name} — ${menuItems.length} menu items`);
  }

  console.log('\n✅ Seeding complete!');

  if (mongod) {
    // If seeding into in-memory DB, it's ephemeral — warn the user
    console.log('\n⚠️  NOTE: You seeded an IN-MEMORY database. Data persists only while the server is running.');
    console.log('   The main server auto-starts its own in-memory DB. Run: npm run dev (from root)');
  }
